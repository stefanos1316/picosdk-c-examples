...
void main (int argc, char** argv)
{
	...
	if ( unitOpened.handle == 0 ) 
	{
		...
		printf ( "Device not found or it might be in used from another application\n" );
	...
	else
	{
		...
		if (argc == 1) {
			...
			printf( "Please use -h to see instructions\n");
			return 0;
		}
		...
		if (!strcmp(argv[1], "-h")) {
			...
			printf ( "\n" );
			printf ( "Please provide only a single action\n");
			printf ( "\n" );
